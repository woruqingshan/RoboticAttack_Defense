"""Image purification for online patch defense.

Applies the chosen strategy (mask_mean, mask_gray, blend_*, blur) to the ROI given by
the controller (PatchBox). Used after the defense step returns should_purify and roi_box
(possibly refined by Step 4). Pure NumPy + optional OpenCV.
"""

from __future__ import annotations

from dataclasses import dataclass
from typing import Optional

import numpy as np

from .anomaly_detector import PatchBox

# Optional dependency; OpenCV is commonly available in this repo environment.
try:
    import cv2  # type: ignore
except Exception:  # pragma: no cover
    cv2 = None


@dataclass
class ImagePurifier:
    """
    Purify an RGB image by modifying a suspected patch region (ROI).

    Backward compatible:
      - purify(image, patch_box) still works exactly as before.

    Supported strategies:
      - mask_mean : overwrite ROI with global mean color (hard).
      - mask_gray : overwrite ROI with fixed gray value (hard).
      - blend_mean: blend ROI toward global mean color (soft, reversible).
      - blend_gray: blend ROI toward gray value (soft, reversible).
      - blur      : blur ROI (soft); can be blended with original by alpha/strength.
    """

    strategy: str = "mask_mean"
    pad: int = 0
    gray_value: int = 127

    # Soft purification controls
    alpha: float = 1.0  # default blend strength (0=no change, 1=full replace)
    blur_ksize: int = 7  # odd integer recommended
    blur_sigma: float = 2.0

    def _prepare_image(self, image: np.ndarray) -> np.ndarray:
        if image.ndim != 3 or image.shape[2] != 3:
            raise ValueError(f"Expected HxWx3 RGB image, got shape={image.shape}")
        if image.dtype != np.uint8:
            return np.clip(image, 0, 255).astype(np.uint8)
        return image

    @staticmethod
    def _normalize_mask(mask_px: np.ndarray, hw: tuple[int, int]) -> np.ndarray:
        if not isinstance(mask_px, np.ndarray):
            raise ValueError("mask_px must be a numpy array")
        if mask_px.ndim != 2:
            raise ValueError(f"mask_px must be 2D, got shape={mask_px.shape}")
        if mask_px.shape != (int(hw[0]), int(hw[1])):
            raise ValueError(f"mask_px shape {mask_px.shape} does not match image shape {hw}")
        return mask_px.astype(bool)

    def purify_with_mask(
        self,
        image: np.ndarray,
        mask_px: np.ndarray,
        strength: Optional[float] = None,
    ) -> np.ndarray:
        """Purify image using a binary pixel mask instead of a box."""
        img = self._prepare_image(image)
        h, w, _ = img.shape
        mask = self._normalize_mask(mask_px, (h, w))
        if not bool(mask.any()):
            return img

        a = self.alpha if strength is None else float(strength)
        a = float(np.clip(a, 0.0, 1.0))
        out = img.copy()

        if self.strategy == "mask_mean":
            mean_color = out.mean(axis=(0, 1)).astype(np.float32)
            if a >= 0.999:
                out[mask] = mean_color.astype(np.uint8)
                return out
            roi_f = out.astype(np.float32)
            tgt = np.broadcast_to(mean_color, out.shape).astype(np.float32)
            blended = (1.0 - a) * roi_f + a * tgt
            out[mask] = np.clip(blended, 0, 255).astype(np.uint8)[mask]
            return out

        if self.strategy == "mask_gray":
            gv = float(np.clip(self.gray_value, 0, 255))
            if a >= 0.999:
                gvu = np.uint8(int(gv))
                out[mask] = np.array([gvu, gvu, gvu], dtype=np.uint8)
                return out
            roi_f = out.astype(np.float32)
            tgt = np.full_like(out, gv, dtype=np.float32)
            blended = (1.0 - a) * roi_f + a * tgt
            out[mask] = np.clip(blended, 0, 255).astype(np.uint8)[mask]
            return out

        if self.strategy == "blend_mean":
            mean_color = out.mean(axis=(0, 1)).astype(np.float32)
            roi_f = out.astype(np.float32)
            tgt = np.broadcast_to(mean_color, out.shape).astype(np.float32)
            blended = (1.0 - a) * roi_f + a * tgt
            out[mask] = np.clip(blended, 0, 255).astype(np.uint8)[mask]
            return out

        if self.strategy == "blend_gray":
            gv = float(np.clip(self.gray_value, 0, 255))
            roi_f = out.astype(np.float32)
            tgt = np.full_like(out, gv, dtype=np.float32)
            blended = (1.0 - a) * roi_f + a * tgt
            out[mask] = np.clip(blended, 0, 255).astype(np.uint8)[mask]
            return out

        if self.strategy == "blur":
            if cv2 is None:
                raise ValueError("purifier strategy 'blur' requires OpenCV (cv2) to be installed.")
            k = int(self.blur_ksize)
            if k <= 1:
                return out
            if k % 2 == 0:
                k += 1
            blurred = cv2.GaussianBlur(out, (k, k), sigmaX=float(self.blur_sigma), sigmaY=float(self.blur_sigma))
            if a >= 0.999:
                out[mask] = blurred[mask]
            else:
                roi_f = out.astype(np.float32)
                blr_f = blurred.astype(np.float32)
                blended = (1.0 - a) * roi_f + a * blr_f
                out[mask] = np.clip(blended, 0, 255).astype(np.uint8)[mask]
            return out

        raise ValueError(f"Unsupported purifier strategy: {self.strategy}")

    def purify(
        self,
        image: np.ndarray,
        patch_box: PatchBox,
        strength: Optional[float] = None,
    ) -> np.ndarray:
        """
        Args:
            image: HxWx3 RGB image (uint8 preferred).
            patch_box: ROI bbox in pixel coordinates.
            strength: Optional override for alpha (0..1). If None, uses self.alpha.

        Returns:
            A purified image (uint8).
        """
        img = self._prepare_image(image)

        h, w, _ = img.shape
        box = patch_box.pad(self.pad).clamp(width=w, height=h)
        if box.x1 <= box.x0 or box.y1 <= box.y0:
            return img

        mask = np.zeros((h, w), dtype=bool)
        mask[box.y0 : box.y1, box.x0 : box.x1] = True
        return self.purify_with_mask(img, mask, strength=strength)
